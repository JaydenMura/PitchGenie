...
export default function ColdEmailGenerator() {
  ...
  const refreshSavedEmails = (updated: SavedEmail[]) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setOutputA("Failed to generate emails. Try again.");
      setOutputB("");
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = (version: "A" | "B") => {
    ...
    const updated = [...savedEmails, newItem];
    refreshSavedEmails(updated);
  ...
  const handleDownloadPDF = async (content?: string, name?: string) => {
    ...
    const fileName = name || recipientName || "cold-email";
    ...
    try {
      ...
      const blob = await pdf(
        <ProposalPDFComponent
          title={`Cold Email for ${fileName}`}
          description={goal || "Email goal not provided"}
          content={pdfContent}
        />
      ).toBlob();
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-6 text-center">Cold Email Generator</h1>
      ...
      <div className="space-y-4">
        ...
        />
        <textarea
          placeholder="Goal of Email (e.g., schedule a meeting)"
          className="w-full border p-3 rounded"
          rows={3}
          value={goal}
        ...
        {advancedMode && (
          <div className="space-y-4 mt-4 p-4 border rounded bg-gray-50">
            <input
              type="text"
              placeholder="Call to Action (e.g., Book a call today)"
            ...
            <input
              type="text"
              placeholder="Reference Point (e.g., common connection, event)"
              className="w-full border p-2 rounded"
      ...
      {savedEmails.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-bold mb-4">Saved Cold Emails</h2>
          <ul className="space-y-3">
            {savedEmails.map((email, index) => (
              ...
              >
                <div>
                  <p className="font-semibold">{email.recipientName}</p>
      ...
      {showModal && selectedEmail && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded shadow-lg w-11/12 max-w-2xl max-h-[90vh] flex flex-col">
            <h3 className="text-xl font-bold p-4 border-b">Saved Cold Email</h3>
            <div className="p-4 overflow-y-auto flex-1">
              <p className="whitespace-pre-line">{selectedEmail.output}</p>
            </div>
            <div className="flex justify-end gap-3 p-4 border-t">
              <button
                onClick={() => handleCopy(selectedEmail.output)}
                className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
              >
                Copy
              </button>
              <button
                onClick={() =>
                  handleDownloadPDF(selectedEmail.output, selectedEmail.recipientName)
                }
